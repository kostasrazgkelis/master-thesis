...
class PipelinePartySerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = PipelineParty
        fields = ["user_uuid", "file_url", "accepted"]
        ...
        for item in value:
            if not isinstance(item, str):
                raise serializers.ValidationError(
                    "All items in match_columns must be strings"
                )
    ...
    def validate_parties(self, value):
        ...
        for item in value:
            if not isinstance(item, int):
                raise serializers.ValidationError(
                    "All items in parties must be integers (user IDs)"
                )

        # Check for duplicates
        if len(value) != len(set(value)):
            raise serializers.ValidationError(
                "parties cannot contain duplicate user IDs"
            )
...
class MatchedDataSerializer(serializers.ModelSerializer):
    """
    Serializer for MatchedData model.
    """

    uuid = serializers.UUIDField(read_only=True)
    left_party = serializers.PrimaryKeyRelatedField(
        read_only=True
    )  # or customize as needed
    right_parties = serializers.PrimaryKeyRelatedField(
    ...
    def validate_right_parties(self, right_parties: list):
        if not right_parties:
            raise serializers.ValidationError("left_parties cannot be empty")
        return right_parties

    def to_representation(self, instance):
        """Return full matched data with user details"""
